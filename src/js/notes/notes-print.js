 ...
 * =============================================================================
 * File: notes-print.js
 * Path: js/notes/notes-print.js
 * Project: Learning Dashboard
 *
 * Description:
 * Custom print system for note pages. Builds and shows the print-options
 * modal (#printModal, opened via window.openPrintModal()), lets the user
 * pick which sections to include, then builds the entire print window's
 * HTML as a string and writes it via document.write() in a new window
 * (no exposed hook to inject extra stylesheets — see dsa-print.js for how
 * DSA pages work around that via a window.open() wrapper).
 *
 * Author: Namrata Mulwani
 * Created: —
 * Last Updated: 2026-07-02
 *
 * Dependencies: none (self-contained; exposes window.openPrintModal for
 * notes-fab.js, notes-accessibility.js, and dsa-print.js to call/patch)
 * =============================================================================
 */
...
(function () {
    ...
    function initPrintAnswerKey() {
        ...
        document.addEventListener("keydown", function (e) {
            if ((e.ctrlKey || e.metaKey) && e.key === "p") {
                e.preventDefault();
                // Go through window.openPrintModal (not the bare local
                // reference) so this respects any patching done by
                // dsa-print.js or similar — same as notes-fab.js's Print
                // menu item already does. Calling the closure variable
                // directly here would always run the original,
                // unpatched version regardless of what overwrote
                // window.openPrintModal later.
    ...
    function openPrintModal() {
        ...
        if (noteContent) {
            noteContent
                .querySelectorAll(":scope > details.collapsible-section")
                .forEach(function (det) {

                    const h2 = det.querySelector(":scope > summary > h2[id]");
                    if (h2) {
                        allSections.push({
                            id: h2.id,
                    ...
                    det.querySelectorAll(":scope > .section-content > details.collapsible-section")
                        .forEach(function (child) {

                            const h3 = child.querySelector(":scope > summary > h3[id]");
                            if (!h3) return;

                            allSections.push({
                                id: h3.id,
                                label: h3.textContent.replace(/[⭐☆▶◀]/g, "").trim(),
                                el: child
                            });
                        });
        ...
        const interactiveSections = [
            ...
            { id: "glossaryContainer", label: "📖 Glossary" },
            {
                ...
                subOptions: [{ id: "interviewAnswers", label: "Include answers" }],
            },
        ].filter(function (s) {
            return !!document.getElementById(s.id);
        });

        // Generic extension point: any details.collapsible-section can be
        // marked data-print-interactive="true" (e.g. by a subject-specific
        // layer like dsa-print.js, before this function runs) to be treated
        // as interactive without this shared file needing to know about
        // subject-specific container ids. No baked-in subOptions here —
        // whatever marked it is expected to add its own sub-toggles onto
        // the modal after openPrintModal() returns, same pattern already
        // used for the fixed four above.
        allSections.forEach(function (s) {
            if (s.el.dataset.printInteractive === "true") {
                interactiveSections.push({ id: s.id, label: s.label });
            }
        });
        ...
        const contentSections = allSections.filter(function (s) {
            ...
            }) && s.el.dataset.printInteractive !== "true";
        });

        // Build toggle pill HTML
        function tog(dataAttr, dataVal) {
            return (
                '<span class="pm-toggle pm-checked" role="checkbox" aria-checked="true" tabindex="0" ' +
                dataAttr +
                '="' +
                dataVal +
                '"></span>'
        ...
        const interactiveHTML = interactiveSections.length
            ...
            interactiveSections
                .map(function (s) {
                    const subs = (s.subOptions || [])
                        .map(function (sub) {
                            return (
                                '<label class="pm-sub-label" data-parent="' +
                                s.id +
                                '">' +
        ...
        const presetBtns = [
            ...
            interactiveSections.length
                ? { p: "interactive", label: "Interactive only" }
                : null,
        ...
        function syncSubs(sectionId, on) {
            modal
                .querySelectorAll('.pm-sub-label[data-parent="' + sectionId + '"]')
        ...
        function syncGroupCB(groupId) {
            const bodyId =
                groupId === "content" ? "#pmContentBody" : "#pmInteractiveBody";
            const body = modal.querySelector(bodyId);
            if (!body) return;
            const anyOn = Array.from(body.querySelectorAll("[data-section]")).some(
                isOn,
            );
        ...
        modal.addEventListener("click", function (e) {
            ...
            if (t.hasAttribute("data-group")) {
                setTog(t, nowOn);
                const bodyId =
                    t.getAttribute("data-group") === "content"
                        ? "#pmContentBody"
                        : "#pmInteractiveBody";
                const body = modal.querySelector(bodyId);
                if (body)
                    body.querySelectorAll("[data-section]").forEach(function (cb) {
                        setTog(cb, nowOn);
            ...
            modal.querySelectorAll(".pm-preset").forEach(function (b) {
        ...
        modal.addEventListener("keydown", function (e) {
            if (
                (e.key === " " || e.key === "Enter") &&
                e.target.classList.contains("pm-toggle")
            ) {
                e.preventDefault();
                e.target.click();
            }
        ...
        function applyPreset(preset) {
            ...
            if (cbContent)
                cbContent.querySelectorAll("[data-section]").forEach(function (cb) {
                    setTog(cb, onContent);
            ...
            if (cbInteractive)
                cbInteractive.querySelectorAll("[data-section]").forEach(function (cb) {
        ...
        modal
            .querySelector(".print-modal-close")
        ...
        modal.querySelector(".pm-print-btn").addEventListener("click", function () {
            ...
            modal
                .querySelectorAll("#pmContentBody [data-section]")
                .forEach(function (t) {
            ...
            modal
                .querySelectorAll("#pmInteractiveBody [data-section]")
                .forEach(function (t) {
    ...
    function executePrint(config) {
        const { content, interactive, opts, contentSections, interactiveSections } =
            config;
        ...
        const printCSSHref = (function () {
            ...
            console.error(
                "❌ Could not resolve notes-print.css path. Is notes.css loaded?",
            );
            return null;
        })();
        ...
        contentSections.forEach(function (s) {
            ...
            clone.querySelectorAll(
                "button, .bookmark-btn, .copy-code-btn"
            ).forEach(function (el) {
                el.remove();
            });

            const heading =
                clone.querySelector(":scope > summary > h2") ||
                clone.querySelector(":scope > summary > h3");
            ...
            if (heading && heading.tagName === "H2") {
                ...
                const body = clone.querySelector(":scope > .section-content");

                if (body) {

                    // Remove nested sections so they aren't printed twice.
                    body.querySelectorAll(":scope > details.collapsible-section")
                        .forEach(function (d) {
                            d.remove();
                        });
                ...
                const body = clone.querySelector(":scope > .section-content");
                bodyHTML = body ? body.innerHTML : clone.innerHTML;
            }

            sectionsHTML +=
                '<div class="print-section">' +
                (headingText
                    ? '<h2 class="print-section-heading">' + headingText + '</h2>'
                    : '') +
        ...
        interactiveSections.forEach(function (s) {
            ...
            const wrapper =
                container.closest("details.collapsible-section") || container;
            ...
            // NOTE: nested <details> (e.g. h3-level subsections inside an
            // h2-level interactive section) are intentionally left as-is
            // here — no longer force-opened. They print in whatever state
            // they're actually in on screen (closed by default), rather
            // than always being forced open. This was previously
            // unconditional for every interactive section; if a future
            // section genuinely needs its nested details force-opened for
            // print, that should be a targeted, section-specific override
            // (e.g. via a dsa-print.js-style hook), not a blanket rule here.

            clone
                .querySelectorAll(".quiz-slide, .flashcard-slide")
            ...
            if (s.id === "quizContainer") {
                clone.querySelectorAll(".quiz-slide").forEach(function (slide) {
                    ...
                    slide.querySelectorAll(".quiz-feedback").forEach(function (fb) {
                        fb.style.display =
                            opts.mcqHighlight || opts.mcqExplain ? "block" : "none";
                    });
            ...
            if (s.id === "interviewContainer") {
                if (!opts.interviewAnswers) {
                    ...
                    clone
                        .querySelectorAll(".flip-card, .qa-card, .interview-card")
            ...
            const innerHTML = sectionContent
            ...
            imgs.forEach(function (img) {
                function removeImg() {
                    ...
                    const wrapper = img.closest(
                        "figure, picture, .image-wrapper, .print-image",
                    );
                    try {
                        (wrapper || img).remove();
                    } catch (e) {
