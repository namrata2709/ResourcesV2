"""
utils/sitemap.py — Shared sitemap helpers
All generator scripts import from here to add/update URLs in sitemap.xml.
Lives alongside the other python/utils/ library modules (animate.py,
parse_*.py) — imported as a module, never run standalone.

Usage:
    from utils.sitemap import add_url, remove_url, write_sitemap, load_sitemap
...
Note: SITEMAP_FILE below is relative to the *working directory* the script
is run from (repo-root-level `python/`), not to this file's own location —
moving this module into utils/ does not change how that path resolves.
"""
