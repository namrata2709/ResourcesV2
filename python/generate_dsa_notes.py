...
def add_meta_seo(fm: dict, cfg: dict):
    ...
    page_url = f"{base_url}dsa-{slug}/notes.html"
...
def add_scripts(cfg: dict):
    depth = cfg["path_depth"]
    return [
        f'    <script src="{depth}js/shared/site-config.js"></script>',
    ]
...
def convert_sections(body_md: str, used_ids: set = None) -> str:
    ...
    def inline_fmt(text: str) -> str:
        # Escape raw text FIRST (handles literal &, <, > typed in prose),
        # then layer markdown syntax on top. html.escape() doesn't touch
        # backticks or [ ] ( ) so it never interferes with the markdown
        # patterns matched below, and it runs before any real tags exist --
        # so those tags can never get re-escaped or corrupted afterward.
